...
public class XmlFactory implements ScenarioFactory {
	...
	public synchronized Scenario getScenario() throws PerfCakeException {
		if (scenario == null) {
			...
			DistributionManager distributionManager = parseDistribution();
			if (distributionManager != null) {
				scenario.setDistributionManager(distributionManager);
				distributionManager.setScenarioModel(scenarioModel);
			}
		...
		if (input != null) {
			...
			try {
				if (input.getAddress() != null
						&& input.getPort() != null) {
					// have address and port
					distributionManager = new DistributionManager(
							InetAddress.getByName(input.getAddress()),
							input.getPort());
				} else if (input.getPort() != null) {
					// port only - listen on localhost
					distributionManager = new DistributionManager(
							InetAddress.getLocalHost(),
							input.getPort());
				} else {
					// fail - cannot continue
					log.fatal("Distribution requires at least a port to listen");
